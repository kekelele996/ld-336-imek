...
func New(deps Deps) *gin.Engine {
	...
	strategyRepo := repository.NewMaintenanceStrategyRepository(deps.DB)
	...
	maintenanceSvc := service.NewMaintenanceService(maintenanceRepo, deviceRepo, strategyRepo, auditSvc, deps.Log)
	strategySvc := service.NewMaintenanceStrategyService(strategyRepo, auditSvc, deps.Log)
	...
	strategyHandler := handler.NewMaintenanceStrategyHandler(strategySvc)
	...
	api := r.Group("/api/v1")
	{
		...
		registerMaintenanceStrategyRoutes(api, strategyHandler)
